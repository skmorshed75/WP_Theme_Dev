...
//Class 2.23
global $mark_section;
$mark_section_meta = get_post_meta( $mark_section['section'], 'mark-section-portfolios', true );
// echo '<pre>';
// print_r($mark_section_meta);
// echo '</pre>';
$mark_images = $mark_section_meta['portfolios'];
$mark_filters = array();
...
?>
				<li><a href="#" data-filter=".<?php echo esc_attr( trim( $mark_filter ) ); ?>"><?php echo esc_html( $mark_filter ); ?></a></li>
				<?php
endforeach;
?>
			</ul>
...
foreach ( $mark_images as $mark_image ):
    ...
    ?>
				<div class="portfolio-item <?php echo esc_attr( strtolower( str_replace( ",", " ", $mark_image['filter'] ) ) ); ?> ">
					<a href="<?php echo esc_url( $mark_image_details_full[0] ); ?>" class="thumb popup-gallery" title="<?php echo esc_attr( $mark_image['title'] ); ?>">
						<img src="<?php echo esc_attr( $mark_image_details_medium[0] ); ?>" alt="<?php echo esc_attr( $mark_image['title'] ); ?>">
